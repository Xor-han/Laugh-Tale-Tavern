...
interface Props {
  ...
  types: Type[];
  onEditSubmit: (id: number, data: CreateFruit) => void;
  onCancel: () => void;
}

export const EditFruitForm = ({ devilFruit, types, onEditSubmit, onCancel }: Props) => {
  ...
  useEffect(() => {
    if (devilFruit) {
      setName(devilFruit.name || "");
      setContent(devilFruit.content || "");
      setTypeId(devilFruit.typeId || null)
    }
  }, [devilFruit]);
 ...
 const handleEditSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !selectedImageId || !type_id || !content.trim()) {
      setError(
        "Le nom, le type, l'image et le contenu sont obligatoires",
      );
      return;
    }
    onEditSubmit(devilFruit.id, {
      name: name.trim(),
      typeId: type_id,
